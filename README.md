...
CMake ≥3.19
QCustomPlot 绘图
BrainFlow DataFilter 预处理
ThinkGear Stream SDK（PC x64）//测试与原文档无关
2 硬件与通信协议
2.1 设备基本参数
通信：蓝牙 2.0 透传 / 串口
波特率：57600
...
2.2.2 大包（因为认为改硬件解析大包的数据不准确，所以只用了里面的signal值，后续需要使用大包数据可以在Steam sdk for pc的文件夹下寻找）
固定头：AA AA 20 ...
长度：32 字节负载 + 头 + 校验
频率：每 512 个小包出现 1 次大包
...
6.2 ThinkGearFrameAssembler（组帧层）
缓存：m_rxBuffer（上限 64KB）
同步头：查找 AA AA
按长度截取完整帧
校验通过后发出 frameReady (frame)
6.3 ThinkGearPayloadParser（解析层）
解析小包：rawdata、校验
解析大包：Signal、Attention、Meditation、8 频段功率
信号：rawReceived、signalQualityReceived、eegPowerReceived
6.4 RawtOutUvProcessor（单位换算）
将 int16 原始值转为微伏（μV）。
6.5 AcquisitionEngine（采集引擎）
封装串口→组帧→解析→换算全链路，输出：
struct RawPacket {QString tsMs; // 时间戳quint64 seq; // 样本序号qint16 rawInt16;quint8 signalQuality;double rawUv;};
6.6 AlgorithmEngine（算法引擎）
分窗：128 点 / 窗
在线采样率估计
预处理管线 EegPreprocessPipeline
...
9 预处理与算法（核心）
9.1 在线采样率估计（解决硬件波动）
fs_inst = dseq * 1000 /dtmsfs_smooth = alpha*fs_smooth + (1-alpha)*fs_instalpha=0.9限幅：200~300Hz
9.2 预处理步骤
1.采样率更新
2.50Hz 陷波（工频）
3.0.5~40Hz 带通
...
10 日志与 CSV 存储
10.1 CSV 列（合并后）
tsMs,seq,rawInt16,signalQuality,rawUv,preprocUv
10.2 写入机制
异步定时 flush（默认几十 ms）
按 seq 合并原始与预处理数据
...
二、安装开发环境（Windows 为例）
安装 Qt 6.5 及以上（与工程 find_package (Qt6 6.5 REQUIRED ...) 版本一致）。安装时勾选：MinGW 64-bit 或 MSVC 2019/2022 64-bit（和后面选的 Kit 一致即可），并勾选：Qt SerialPort、Qt 5/6 的 CMake 相关组件（Qt Creator 一般会带上）。
...
5.左下角点锤子（构建），再点运行。
6.构建成功后，thinkgear64.dll 和 DataHandler.dll 会按工程里的 POST_BUILD 规则拷贝到 exe 同目录，一般可直接运行。
...
测试1：stream_sdk_for_pc(官方开发包)里面现成的函数可以直接调用，参考thinkgear.h（这是一个API文档）,测试——stream_sdk_for_pc/origintest.h
...
所以先做一个线估采样率的算法，他的作用是：让这些处理在设备波动时仍然稳定有效。
数学公式如下
...
限幅防异常：
把 fs_inst 限在 [fsmin,fsmax]（你设的是 200~300）
代码在EegPreprocessPipeline类里面，在这个类里面有三个结构体，第一个结构体，Options（用来设置采样率估计，滤波参数，降采样，单通道伪影，小波去噪的参数），这些参数有的是利用了（BrainFlow的开源代码，一个预处理的开源库）
...
updateFsEstimate(hint, dbg)：尝试在线估计采样率。
fs = round(mfssmooth)，保存到 out.fs_used。
...
然后 m_timer.start(mflushIntervalMs)意思是，每隔固定时间触发 onFlushTick()，
...
组帧字节缓冲	ThinkGearFrameAssembler::m_rxBuffer	超过 64 KB 整段清空并计数溢出	采集	处理粘包 / 半包，拼接完整 ThinkGear 帧
...
2.2 ThinkGearFrameAssembler::m_rxBuffer（上限 64 KB）
